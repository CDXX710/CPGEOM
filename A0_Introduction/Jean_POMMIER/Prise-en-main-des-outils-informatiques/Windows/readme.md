...
- [ ] Arborescence système
  - [ ] bonnes pratiques
  - [ ] trouver un fichier
  - [ ] créer un fichier/dossier - avec la souris - avec le clavier
...
- une IDE (_integrated development environment_) qui fournit une interface plus riche, avec des assistants, raccourcis etc. Tout un environnement dédié en général à un langage, framework ou flux de développement. Une IDE permet en général de travailler plus efficacement mais nécessite un temps de prise en main.
...
- Dans ce même fichier XML, un `s` s'est glissé à la fin des prénoms aux lignes 11, 20 et 24. Supprimez ces trois `s` _en une seule édition_
...
_Mais je n'ai pas trouvé de moyen de désactiver la recherche dans la barre d'adresse_...
...
- `cd` : se déplacer (`cd` = _change directory_).
...
_Sujet suivant : [linux](../linux/readme.md)_
